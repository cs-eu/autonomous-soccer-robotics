...
void VL53L1X::writeReg(uint16_t reg, uint8_t value)
{
        ...
        Wire2.write((reg >> 8) & 0xFF); // reg high byte
        Wire2.write( reg       & 0xFF);// reg low byte
        Wire2.write(value);
...
{
        Wire2.beginTransmission(address);
        Wire2.write((RESULT__RANGE_STATUS >> 8) & 0xFF); // reg high byte
        Wire2.write( RESULT__RANGE_STATUS       & 0xFF);// reg low byte
        last_status = Wire2.endTransmission();

        Wire2.requestFrom(address, (uint8_t)17);

        results.range_status = Wire2.read();

        Wire2.read(); // report_status: not used

        results.stream_count = Wire2.read();

        results.dss_actual_effective_spads_sd0  = (uint16_t)Wire2.read() << 8;// high byte
        results.dss_actual_effective_spads_sd0 |=           Wire2.read();// low byte

        Wire2.read(); // peak_signal_count_rate_mcps_sd0: not used
        Wire2.read();

        results.ambient_count_rate_mcps_sd0  = (uint16_t)Wire2.read() << 8;// high byte
        results.ambient_count_rate_mcps_sd0 |=           Wire2.read();// low byte

        Wire2.read(); // sigma_sd0: not used
        Wire2.read();

        Wire2.read(); // phase_sd0: not used
        Wire2.read();

        results.final_crosstalk_corrected_range_mm_sd0  = (uint16_t)Wire2.read() << 8;// high byte
        results.final_crosstalk_corrected_range_mm_sd0 |=           Wire2.read();// low byte

        results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0  = (uint16_t)Wire2.read() << 8;// high byte
        results.peak_signal_count_rate_crosstalk_corrected_mcps_sd0 |=           Wire2.read();// low byte
}
